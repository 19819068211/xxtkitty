...
def construct_questions_form(questions: list[QuestionModel]) -> dict[str, int | str]:
    ...
    """
    form = {
        "answerwqbid": ",".join(str(q.id) for q in questions)
    }
...
class PointWorkDto(QAQDtoBase):
    ...
    questions: list[QuestionModel]  # 当前作业的全部题目
    ...
    def final_submit(self) -> dict:
        ...
        resp = self.session.post(
            ...
            data={
                ...
                "userId": self.acc.puid,
                "workTimesEnc": "",
                **construct_questions_form(self.questions), # 构建答题提交表单
            },
        )
        resp.raise_for_status()
        ...
        if (json_content.get("status")) != True:
            self.logger.error(f"交卷失败 ({json_content.get('msg')}) [{self.title}(J.{self.job_id}/W.{self.work_id})]")
            raise PointWorkError(json_content.get("msg"))
        
        self.logger.info(f"交卷成功 ({json_content.get('msg')}) [{self.title}(J.{self.job_id}/W.{self.work_id})]")
        return json_content
    ...
    def fallback_save(self) -> dict:
        ...
        # 解析失败参数
        if (json_content.get("status")) != True:
            self.logger.error(f"保存失败 ({json_content.get('msg')}) [{self.title}(J.{self.job_id}/W.{self.work_id})]")
            raise PointWorkError(json_content.get("msg"))
